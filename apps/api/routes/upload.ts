...
const PUBLIC_URL = process.env.PUBLIC_URL || '/uploads'
...
export const uploadRoutes = new Elysia({ prefix: '/upload' }).post(
  ...
  async ({ body, set }) => {
    try {
      ...
      const fileId = nanoid()
      const outputFormat = ['webp', 'avif', 'jpeg', 'png'].includes(format) ? format : 'webp'
      const filename = `${fileId}.${outputFormat}`
      const outputPath = join(UPLOAD_DIR, filename)

      console.log(`Processing image: sourceFormat=${sourceFormat}, targetFormat=${outputFormat}, size=${file.size}`)
      ...
      if (sourceFormat === 'avif') {
        try {
          console.log('Processing AVIF through intermediate JPEG for better compatibility')
          ...
          try {
            ...
            console.log('AVIF intermediate conversion through temp file successful')
          } catch (tempFileErr) {
            console.error('Temp file approach failed:', tempFileErr)

            // Fall back to in-memory conversion as before
            const jpegBuffer = await sharp(originalBuffer).jpeg({ quality: 90 }).toBuffer()
          ...
          console.log(`Successfully decoded AVIF through JPEG, will now convert to final format: ${outputFormat}`)
        } catch (avifErr) {
      ...
      try {
        console.log(`Applying final format conversion to: ${outputFormat} with quality: ${quality}`)
        const outputBuffer = await sharpInstance.toBuffer()
        console.log(`Format conversion successful, buffer size: ${outputBuffer.length}`)
        ...
        console.log(`Image processed successfully: id=${fileId}, format=${outputFormat}, size=${outputBuffer.length}`)
